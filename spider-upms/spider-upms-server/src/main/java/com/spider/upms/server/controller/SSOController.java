package com.spider.upms.server.controller;

import com.spider.common.base.BaseController;
import com.spider.common.util.RedisUtil;
import com.spider.upms.client.shiro.session.UpmsSession;
import com.spider.upms.client.shiro.session.UpmsSessionDao;
import com.spider.upms.common.constant.UpmsResult;
import com.spider.upms.common.constant.UpmsResultConstant;
...
public class SSOController extends BaseController {
    ...
    public String login(HttpServletRequest request) {
        ...
        // 判断是否已登录，如果已登录，则回跳
        String code = RedisUtil.get(SPIDER_UPMS_SERVER_SESSION_ID + "_" + serverSessionId);
        // code校验值
    ...
    public Object login(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) {
        ...
        String hasCode = RedisUtil.get(SPIDER_UPMS_SERVER_SESSION_ID + "_" + sessionId);
        // code校验值
        if (StringUtils.isBlank(hasCode)) {
            ...
            RedisUtil.lpush(SPIDER_UPMS_SERVER_SESSION_IDS, sessionId.toString());
            // 默认验证帐号密码正确，创建code
            String code = UUID.randomUUID().toString();
            // 全局会话的code
            RedisUtil.set(SPIDER_UPMS_SERVER_SESSION_ID + "_" + sessionId, code, (int) subject.getSession().getTimeout() / 1000);
            // code校验值
            RedisUtil.set(SPIDER_UPMS_SERVER_CODE + "_" + code, code, (int) subject.getSession().getTimeout() / 1000);
        }
        // 回跳登录前地址
        String backurl = request.getParameter("backurl");
        if (StringUtils.isBlank(backurl)) {
    ...
    public Object code(HttpServletRequest request) {
        ...
        String code = RedisUtil.get(SPIDER_UPMS_SERVER_CODE + "_" + codeParam);
